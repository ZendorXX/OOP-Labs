...
TEST(test_triangles, test_center_1) {
    Point2D points[3] = { {0, 0}, {3, 0}, {0, 4} };
    Figure *A = new Triangle(points);
    ASSERT_TRUE (A->geometrical_center() == Point2D({1, double(4) / double(3)}) );
}

TEST(test_triangles, test_center_2) {
    Point2D points[3] = { {0, 0}, {5, 0}, {0, 6} };
    Figure *A = new Triangle(points);
    ASSERT_TRUE (A->geometrical_center() == Point2D({double(5) / double(3), 2}) );
}

TEST(test_triangles, test_equal_1) {
    Point2D points[3] = { {0, 0}, {5, 0}, {0, 6} };
    Point2D points2[3] = { {0, 6}, {0, 0}, {5, 0} };
    Figure* A = new Triangle(points);
